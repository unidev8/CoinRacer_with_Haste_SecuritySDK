...
public class AICarTrack : MonoBehaviour
{
    ...
    private void Start()
    {
        CurrentPoint = StartPoint;//at the start of the race the AI car will drive to the first AI waypoint object
    ...
    {
        if (collision.attachedRigidbody.gameObject.name == AICarName)//If the AICar collider is detected
        {
            Point = GameObject.Find("Point" + CurrentPoint);    //the script will search the next point automatically
            this.transform.position = Point.transform.position;// + new Vector3 (Random.Range (-1f, 1f), 0.0f, Random.Range(-1f, 1f)); //then, it will get the position of the point and move the tracker to there 
